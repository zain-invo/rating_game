defmodule RatingGameWeb.RatingLive.Index do
  ...
  def list(assigns) do
    ...
    <%= for {game, index} <- Enum.with_index(@games) do %>
      <%= if rating = List.first(game.ratings) do %>
        <RatingGameWeb.RatingLive.Show.stars rating={rating} game={game} />
      <% else %>
        <.live_component module={RatingGameWeb.RatingLive.Form}
